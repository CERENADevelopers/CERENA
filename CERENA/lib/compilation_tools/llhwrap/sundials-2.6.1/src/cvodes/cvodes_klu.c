...
int CVKLUSetOrderingB(void *cvB_mem_v, int which, int ordering_choice)
{
    CVodeMem cv_mem;
    CVadjMem ca_mem;
    CVodeBMem cvB_mem;
    ...
    /* Return immediately if cv_mem is NULL */
    if (cvB_mem_v == NULL) {
        cvProcessError(NULL, CVSLS_MEM_NULL, "CVSLS", "CVKLUSetOrderingB",
                       MSGSP_CVMEM_NULL);
        return(CVSLS_MEM_NULL);
    }
    cv_mem = (CVodeMem) cvB_mem_v;
    
    /* Was ASA initialized? */
    if (cv_mem->cv_adjMallocDone == FALSE) {
        cvProcessError(cv_mem, CVSLS_NO_ADJ, "CVSLS", "CVKLUSetOrderingB", MSGSP_NO_ADJ);
        return(CVSLS_NO_ADJ);
    }
    ca_mem = cv_mem->cv_adj_mem;
    ...
    /* Check which */
    if ( which >= ca_mem->ca_nbckpbs ) {
        cvProcessError(cv_mem, CVSLS_ILL_INPUT, "CVSLS", "CVKLUSetOrderingB", MSGSP_BAD_WHICH);
        return(CVSLS_ILL_INPUT);
    }
    ...
    if ( (ordering_choice != 0) && (ordering_choice != 1) &&
        (ordering_choice != 2) ) {
        cvProcessError(NULL, CVSLS_ILL_INPUT, "CVSLS", "CVKLUSetOrderingB",
                       MSGSP_ILL_INPUT);
        return(CVSLS_ILL_INPUT);
    }
    ...
    cvslsB_mem = (CVSlsMemB) cvB_mem->cv_lmem;
    klu_data = (KLUData) cvslsB_mem->s_solver_data;
