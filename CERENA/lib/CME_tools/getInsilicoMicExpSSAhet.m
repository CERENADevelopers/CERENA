...
% USAGE:
% ======
% [measurement] = getInsilicoMicExpSSA(model,timevector, parDist)
% [measurement] = getInsilicoMicExpSSA(model,timevector,parDist,measurands)
% [measurement] = getInsilicoMicExpSSA(model,timevector,parDist,measurands,options)
%
% INPUTS:
% ======
...
% parDist ... parameters of the loglikelihood parameter distribution 
%             parDist=[mu, sigma]
...
% function measurement = getInsilicoMicExpSSA(model,timevector,parDist,measurands,options)
...
%% SIMULATE SSA
for i=1:options.Nt
X = simulateSSAcontinuous(@(x) v(x, lognrnd(parDist(1), parDist(2))),S,timevector,x0);
