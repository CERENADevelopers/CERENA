...
c = sym('c',size(system.parameter.variable));
nx = length(xmin);
...
for i = 1:length(system.reaction)
    ...
    rate = sym2fun(simplify(system.reaction(i).propensity/system.reaction(i).parameter),x);
    % Initialize matrix:
    system.A_i{i} = sparse(zeros(nindex));
    % Construction of A^{(i)} column-wise:
    for j = 1:nindex
       system.A_i{i}(j,j) = - rate(index(j,:)');
       ...
       % Assign reaction
       if ~isempty(j2)
           system.A_i{i}(j2,j) = rate(index(j,:)');
       end
    end
    % Assign coefficient:
...
% end
for i = 2:length(system.reaction)
    expr = [expr '+ system.sigma_A_i{' num2str(i) ' }(theta)*system.A_i{' num2str(i) '}'];
end
expr = [expr '];'];
