...
% function System = genSimFile(modelName,modelDefFileName,simMethod,System)
function System = genSimFile(varargin)
    if nargin >=3
        modelName = varargin{1};
        modelDefFileName = varargin{2};
        simMethod = varargin{3};
    else
            error('At least three input arguments are required!')
    end
    if nargin >= 4
        System = varargin{4};
        System = genmexp(modelName,modelDefFileName,simMethod,System);
    else
        System = genmexp(modelName,modelDefFileName,simMethod);
    end
    cvodewrap(modelName,[simMethod,'_',modelName,'_syms'])
end
