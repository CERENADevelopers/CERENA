...
%             System.scaleConversion = 'Macro_to_Micro';
    ...
    if length(System.state.variable) <= 40
        options.sym_kappa = true;
    else
        options.sym_kappa = false;
    end
    System = completeSystem(System,options);
...
if(strcmp(expansion(1:2),'ME'))
    ...
    if(strcmp(expansion(3:4),'C_'))
        ...
        if length(expansion) > 12
            covReduc = expansion(14);
%             covReduc = expansion(14:15);
        else
            covReduc = 'f';
        end
        switch covReduc
            case 'n'
                options.covReduc = true;
                options.covRed_closure = 'normal';
            case 's'
                options.covReduc = true;
                options.covRed_closure = 'special';
            case 'f'
                options.covReduc = false;  
            otherwise
%                 options.covReduc = str2double(covReduc);
                options.covReduc = covReduc;
        end
        if length(expansion) > 15
            options.moment_order_max = str2double(expansion(16));
%             options.moment_order_max = str2double(expansion(17));
        end
        ...
        System = completeSystem(System,options);
        System.name = 'MM';
        if strcmp(covReduc,'g')||strcmp(covReduc,'n') || strcmp(covReduc,'s') % Reduction based on reaction graph
            if isfield(System,'reduction_order')
                options.reduction_order = System.reduction_order;
            else
                options.reduction_order = 1;
            end
            MM = getMM_centered_reduced_net(System,options);
%                     elseif isnumeric(options.covReduc)
        elseif strcmp(covReduc,'r')
            MM = getMM_centered_rand_net(System,options);
        elseif strcmp(covReduc,'a')
            MM = getMM_centered_greedy(System,options);
        else
