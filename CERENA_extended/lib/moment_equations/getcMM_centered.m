...
for j = 1:n_r
    % c
    rate(j) = prod(mysubs(system.reaction(j).parameter,system.parameter.variable,c));
...
%             hy{j} = @(y) y(ind)*(y(ind)-1)/2;
        ...
        case 3
            ind = find(S_ey(:,j)==3);
%             hy{j} = @(y) y(ind)*(y(ind)-1)*(y(ind)-2)/6;
            hy{j} = @(y) y(ind)^3;
    end
...
%             hz{j} = z(ind)*(z(ind)-1)/2;
        ...
        case 3
            ind = find(S_ez(:,j)==3);
%             hz{j} = z(ind)*(z(ind)-1)*(z(ind)-2)/6;
            hz{j} = z(ind)^3;
    end
    else
        hy{j} = @(y) 1;
        hz{j} = mysubs(mysubs(system.reaction(j).propensity,system.state.variable(n_y+1:end),z),system.parameter.variable,c)/rate(j);
    end
