...
ind_hoM_used2 = 1:length(hoM_used2);
% hoM_closure2 = sym([]);
...
if ~isempty(hoM_used2)
    ...
    if strcmp(options.moment_closure,'user-defined')
        Ii = hoI(ismember(hoM,hoM_used2),:);
        nargout_ud = nargout('ud_closure');
        if nargout_ud == 1
            hoM_closure2 = ud_closure(Ii,options.moment_order,system);
            ind_hoM_used2 = [];
        elseif nargout_ud == 2
            [hoM_closure_user,flag_closure] = ud_closure(Ii,options.moment_order,system);
            hoM_closure2(flag_closure~=0)= hoM_closure_user(flag_closure~=0);
            ind_hoM_used2 = transpose(find(flag_closure==0));
