...
function MM = getMM_centered_rand_net(varargin)
...
%% INITIALIZATION (2)
% get Reaction Graph
% system = getReactionGraph(system);
% Indices of the *2nd-order* significant covariances, equivalent to the
% edges in the network:
I = [zeros(n_s,1),[1:n_s]'];
I = [I;[[1:n_s]',[1:n_s]']];
if isfield (options,'covReducInd')
    I = [I;options.covReducInd];
else
    % [I1,I2] = find(triu(system.reactionGraph));
    % tmp = [I1,I2];
    tmp = randi(n_s,options.covReduc,2);
    tmpind = find(tmp(:,1)==tmp(:,2));
    ...
    tmpind = find(tmp(:,1)>tmp(:,2));
    tmptmpind = tmp(tmpind,1);
    tmp(tmpind,1) = tmp(tmpind,2);
    tmp(tmpind,2) = tmptmpind;
    I = [I;tmp];
