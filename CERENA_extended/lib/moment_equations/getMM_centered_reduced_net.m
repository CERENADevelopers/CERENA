...
function MM = getMM_centered_reduced_net(varargin)
...
%% INITIALIZATION (2)
% get Reaction Graph
system = getReactionGraph(system,options);
% Indices of the *2nd-order* significant covariances, equivalent to the
% edges in the network:
I = [zeros(n_s,1),[1:n_s]'];
[I1,I2] = find(triu(system.reactionGraph));
tmp = [I1,I2];
I = [I;tmp];
if isfield(system,'addcovar')
    I = union(I,system.addcovar,'rows');
end
if isfield(system,'conservedSp')
    v = system.conservedSp;
    I_conserved = v;
    I_conserved = [kron(I_conserved,ones(length(v),1)),kron(ones(size(I_conserved,1),1),v)];
    % Sorting and elimination of redundant moments
    I_conserved = unique(sort(I_conserved,2),'rows');
    I = union(I,I_conserved,'rows');
end

if options.moment_order == 3
    
    I = [zeros(size(I,1),1),I];
        tmp = repmat([1:n_s]',1,3);
        I = [I;tmp];
    if ~isempty(system.I_o3)
...
[hoI,ind_hoI] = setdiff(Iall,I,'Rows');
ind_I = setdiff([1:size(Iall,1)]',ind_hoI);
...
%         M0(1:length(system.state.mu0)+length(system.state.C0)) = [system.state.mu0;system.state.C0];
        M0 = sym(zeros(size(Iall,1),1));
        tmp = [system.state.mu0;system.state.C0];
        M0(1:length(tmp)) = tmp;
        M0 = M0(ind_I);
        M0 = mysubs(M0,system.parameter.variable,c);
        M0 = mysubs(M0,system.kappa.variable,c_kappa);
...
%     dwdx{r} = simplify(jacobian(w{r},mu));
    dwdx = simplify(jacobian(w{r},mu));
%     d2wdx2{r} = simplify(hessian(w{r},mu));
    d2wdx2 = simplify(hessian(w{r},mu));
    %
    if w{r} ~= 0
        term{r}(1) = w{r};
        L{r}(1,1:2) = 0;
...
%                     term{r}(end+1) = 0.5*d2wdx2{r}(k,l);
                    term{r}(end+1) = 0.5*d2wdx2(k,l);
                else
%                     term{r}(end+1) = d2wdx2{r}(k,l);
                    term{r}(end+1) = d2wdx2(k,l);
                end
                L{r}(end+1,1:2) = [k,l];
            end
        end
...
%     if (options.covReduc)
%         ind_var = find((I(:,end-1)==I(:,end)).*(sum(I~=0,2)==2));
...
% %         end
% %         ind_M = sort(ind_M);
%         [~,ind_M] = ismember(M_reduced,M);
%         ind_M = sort(ind_M(ind_M~=0));
...
%         hoI = [IAll(ind_hoM,:);hoI];
    if isfield(options,'covRed_closure')
        if strcmp(options.covRed_closure,'special')
            var_system = [c;c_kappa;M;system.time];
            ind_hoM1 = ismember(hoM,setdiff(symvar(dMdt),M));
            hoM1 = hoM(ind_hoM1);
            hoI1 = hoI(ind_hoM1,:);
            hoM_closure1 = covRedMomClosure(hoI1,system);
            hoM_closure1 = mysubs(hoM_closure1,system.parameter.variable,c);
            hoM_closure1 = mysubs(hoM_closure1,system.kappa.variable,c_kappa);
            while ~isempty(setdiff(symvar(hoM_closure1),var_system))
...
var_system = [c;c_kappa;M2;system.time];
if isfield(system,'input')
...
hoM_used2 = transpose(setdiff(symvar(dMdt),var_system));
hoM_closure2 = sym([]);
        ...
        for i = 1:length(hoM_used2)
            ...
            switch options.moment_closure
                ...
                case 'derivative-matching'
                    ...
                    hoM_closure2(i) = hoM_closure2(i) + prod(uncentMom.^gamma_p);
                case 'log-normal'
                    Ii = hoI(j,:);
                    alpha_bar = convertI2alpha(Ii,n_s);
                    hoM_uncent = convertUncent2Cent(Ii,alpha_bar,mu,n_s);
...
hoM_used = [transpose(hoM1),transpose(hoM_used2)];
hoM_closure = [transpose(hoM_closure1),transpose(hoM_closure2)];
...
% Substitutions
var_system_subs = [c;c_kappa;M;system.time];
if isfield(system,'input')
    var_input = [system.input.variable;transpose(symvar(system.input.function))];
    var_system_subs = [var_system_subs;var_input];
end
if isfield(system,'output')
    var_output = [system.output.variable;transpose(symvar(system.output.function))];
    var_system_subs = [var_system_subs;var_output];
end
while (~isempty(setdiff(symvar(dMdt),var_system_subs)))
    dMdt = mysubs(dMdt,hoM_used,hoM_closure);
end

while (~isempty(setdiff(symvar(H),var_system_subs,'legacy')))
