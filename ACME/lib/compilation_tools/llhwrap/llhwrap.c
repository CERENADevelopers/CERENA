...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    int cv_status;
    double *status;
    int np, nx, ny, nt, nr; /* integers with problem dimensionality */
    ...
    double *llhdata, *llhSdata, *llhS2data;
    double g, *dgdp, *dgdx;
    double r, *drdp, *drdx;
...
*/
    ...
    if (nx > 0) {
        ...
        if(nr>0) rootsfound = mxMalloc(nr*sizeof(double));
        ...
        *status = CVodeSetStabLimDet(cvode_mem, stldet); /* activates stability limit detection */
        ...
        if (nr>0) {
            *status = cvodewrap_RootInit(cvode_mem, nr); /* activates root detection */
            if(*status != CV_SUCCESS) goto error_return;
        }
                                            ...
                                            if(rootsfound[ir] == 1) {
                                                ...
                                                if(!mxIsNaN(mt[ir*nmaxroot+nroots])) {
                                                    ...
                                                    if (event_model == LW_LOGNORMAL) {
                                                        r += 0.5*log(2*pi*pow(tsigma[ir*nmaxroot+nroots]*t,2)) + 0.5*pow( ( log(t) - log(mt[ir*nmaxroot+nroots]) )/tsigma[ir*nmaxroot+nroots] , 2);
                                                    }
                                                    if (event_model == LW_NORMAL) {
                                                        r += 0.5*log(2*pi*pow(tsigma[ir*nmaxroot+nroots],2)) + 0.5*pow( ( rootvaltmp[ir] )/tsigma[ir*nmaxroot+nroots] , 2);
                                                    }
                                                    if (event_model == LW_LOGNORMAL) {
                                                        r += 0.5*log(2*pi*pow(tsigma[ir*nmaxroot+nroots]*0,2)) + 0.5*pow( ( log(0) )/tsigma[ir*nmaxroot+nroots] , 2);
                                                    }
                                                    if (sensi>=1) {
                                                        x_tmp = NV_DATA_S(x);
                                                        ...
                                                        for (ip=0; ip<np; ip++) {
                                                            ...
                                                            if(event_model  == LW_LOGNORMAL) {
                                                                drdp[ip] += 1/(2*pi)*drvaldp[ip*nr+ir]/0 + ( drvaldp[ip*nr+ir]/0 * ( log(0) ) )/pow( tsigma[ir*nmaxroot+nroots] , 2);
                                                            }
                                                        }
                                                        for (ix=0; ix<nx; ix++) {
                                                            if(event_model  == LW_NORMAL) {
                                                            ...
                                                            if(event_model  == LW_LOGNORMAL) {
                                                                drdx[nroots+ix*nmaxroot] += 1/(2*pi)*drvaldx[ix*nr+ir]/0 + ( drvaldx[ix*nr+ir]/0 * ( log(0) ) )/pow( tsigma[ir*nmaxroot+nroots] , 2);
                                                            }
                                                        }
                                                    }
                                                }
        ...
        if (nroots==0) {
            for (ir=0; ir<nr; ir++){
                ...
                if(!mxIsNaN(mt[ir*nmaxroot+nroots])) {
                    ...
                    if (event_model == LW_LOGNORMAL) {
                        r += 0.5*log(2*pi*pow(tsigma[ir*nmaxroot+nroots]*rootvaltmp[ir],2)) + 0.5*pow( ( log(rootvaltmp[ir]) )/tsigma[ir*nmaxroot+nroots] , 2);
                    }
                    if (sensi>=1) {
                        x_tmp = NV_DATA_S(x);
                        fdtdp(t,dtdp,data->p,data->k,x_tmp,plist,np,nmaxroot);
            ...
            if(cv_status == 0.0) {
                ...
                *status = CVodeSetMaxNumStepsB(cvode_mem, which, cvodes_maxsteps);
                ...
                switch (linsol) {
                    ...
                    case LW_BAND:
                        /**status = CVBandB(cvode_mem, which, nx);*/
                        *status = -1;
                        if (*status != CV_SUCCESS) goto error_return;
                    ...
                    case LW_LAPACKBAND:
                        /**status = CVLapackBandB(cvode_mem, which, nx);*/
                        *status = -1;
                        if (*status != CV_SUCCESS) goto error_return;
