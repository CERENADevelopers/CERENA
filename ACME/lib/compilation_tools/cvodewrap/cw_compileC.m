...
for j=1:length(sources_sundials)
    if(~exist(fullfile(odewrap_path,'models',mexext,objects_sundials{j}), 'file') || del_sundials)
        eval(['mex -c -outdir ' fullfile(odewrap_path,'models',mexext) includesstr ' ' fullfile(sundials_path,sources_sundials{j})]);
    end
end

for j=1:length(sources_ssparse)
    if(~exist(fullfile(odewrap_path,'models',mexext,objects_ssparse{j}), 'file') || del_ssparse)
        eval(['mex -c -outdir ' fullfile(odewrap_path,'models',mexext) includesstr ' ' fullfile(ssparse_path,sources_ssparse{j})]);
    end
end

eval(['mex COPTIMFLAGS=''-Ofast -DNDEBUG'' -output ' fullfile(odewrap_path,'models',filename,['cw_' filename]) ' ' fullfile(odewrap_path,'cvodewrap.c')  includesstr objectsstr ])
