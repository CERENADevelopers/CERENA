...
elseif(strcmp(svar,'sx'))
    ...
    if(any(struct.sym.sx(:,ip)~=0))
        gccode(struct.sym.sx(:,ip),cvar,fid,splineflag);
    end
elseif(strcmp(svar,'sx0'))
    cvar =  'sx0_tmp';
...
elseif(strcmp(svar,'sy'))
    ...
    if(any(struct.sym.sy(:,ip)~=0))
        gccode(struct.sym.sy(:,ip),cvar,fid,splineflag);
    end
elseif(strcmp(svar,'sroot'))
    cvar =  'dr_dp';
    ...
    if(any(nonzero))
        ...
        for ir=1:nr
            if(nonzero(ir))
                ...
                gccode(struct.sym.sroot(ir,ip),cvar,fid,splineflag);
                fprintf(fid,'\n');
                fprintf(fid,'          } break;\n\n');
            end
        end
...
elseif(strcmp(svar,'s2root'))
    ...
    if(any(nonzero))
        ...
        for ir=1:nr
            if(nonzero(ir))
                fprintf(fid,['              case ' num2str(ir-1) ': {\n']);
                gccode(struct.sym.s2root(ir,ip,jp),cvar,fid,splineflag);
                fprintf(fid,'\n');
                fprintf(fid,'              } break;\n\n');
            end
        end
...
elseif(strcmp(svar,'srootval'))
    ...
    if(any(nonzero))
        ...
        for ir=1:nr
            if(nonzero(ir))
                fprintf(fid,['          case ' num2str(ir-1) ': {\n']);
                gccode(struct.sym.srootval(ir,ip),cvar,fid,splineflag);
                fprintf(fid,'\n');
                fprintf(fid,'          } break;\n\n');
            end
        end
...
elseif(strcmp(svar,'s2rootval'))
    ...
    if(any(nonzero))
        ...
        for ir=1:nr
            if(nonzero(ir)~=0)
                fprintf(fid,['              case ' num2str(ir-1) ': {\n']);
                gccode(struct.sym.s2rootval(ir,ip,jp),cvar,fid,splineflag);
                fprintf(fid,'\n');
                fprintf(fid,'              } break;\n\n');
            end
        end
...
%         cstr = [ cvar '[' num2str(j-1) '] ' tmp_str(5:end) ';'];
    ...
    if(~(length(cstr)==1 && isempty(cstr{1})))
        if(strcmp(cvar,'y') || strcmp(cvar,'sy') || strcmp(cvar,'dydp'))
            if(strcmp(cvar,'y') || strcmp(cvar,'dydp'))
