% simulate_geneExp_LNA.m is the matlab interface to the cvodes mex
%   which simulates the ordinary differential equation and respective
%   sensitivities according to user specifications.
%
% USAGE:
% ======
% [...] = simulate_geneExp_LNA(tout,theta)
% [...] = simulate_geneExp_LNA(tout,theta,kappa,options)
% sol = simulate_geneExp_LNA(...)
% [status,tout,x,mx,y,sx,smx,sy] = simulate_geneExp_LNA(...)
%
% INPUTS:
% =======
...
function varargout = simulate_geneExp_LNA(varargin)
...
options_cvode.nnz = 65; % MUST NOT CHANGE THIS VALUE
...
cw_geneExp_LNA(sol,tout,theta(1:10),kappa(1:29),options_cvode,plist,pbar,xscale);
rt = [7   8   4  14   1   9   2  10  11   3  12  13   6   5];
sol.x = sol.x(:,rt);
