% simulate_geneExp_RRE.m is the matlab interface to the cvodes mex
%   which simulates the ordinary differential equation and respective
%   sensitivities according to user specifications.
%
% USAGE:
% ======
% [...] = simulate_geneExp_RRE(tout,theta)
% [...] = simulate_geneExp_RRE(tout,theta,kappa,options)
% sol = simulate_geneExp_RRE(...)
% [status,tout,x,mx,y,sx,smx,sy] = simulate_geneExp_RRE(...)
%
% INPUTS:
% =======
...
function varargout = simulate_geneExp_RRE(varargin)
...
options_cvode.nx = 4; % MUST NOT CHANGE THIS VALUE
options_cvode.ny = 5; % MUST NOT CHANGE THIS VALUE
options_cvode.nr = 0; % MUST NOT CHANGE THIS VALUE
options_cvode.ndisc = 0; % MUST NOT CHANGE THIS VALUE
options_cvode.nnz = 10; % MUST NOT CHANGE THIS VALUE
...
options_cvode.qPositiveX = zeros(length(tout),4);

sol.status = 0;
sol.t = tout;
sol.x = zeros(length(tout),4);
sol.y = zeros(length(tout),5);
sol.xdot = zeros(length(tout),4);
...
if(options_cvode.sensi>0)
    sol.xS = zeros(length(tout),4,length(options_cvode.sens_ind));
    sol.yS = zeros(length(tout),5,length(options_cvode.sens_ind));
...
cw_geneExp_RRE(sol,tout,theta(1:10),kappa(1:29),options_cvode,plist,pbar,xscale);
rt = [2  3  1  4];
sol.x = sol.x(:,rt);
...
if(nargout>1)
    ...
    varargout{2} = sol.t;
    varargout{3} = sol.x;
    varargout{4} = sol.y(:,1:4); % Moments of species
    varargout{5} = sol.y(:,5:end);
    if(nargout>5)
        varargout{6} = sol.sx;
        varargout{7} = sol.sy(:,1:4,:);
        varargout{8} = sol.sy(:,5:end,:);
    end
else
    sol.mx = sol.y(:,1:4);
    sol.y =  sol.y(:,5:end);
