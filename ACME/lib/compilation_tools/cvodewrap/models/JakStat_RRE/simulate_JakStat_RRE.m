% simulate_JakStat_RRE.m is the matlab interface to the cvodes mex
%   which simulates the ordinary differential equation and respective
%   sensitivities according to user specifications.
%
% USAGE:
% ======
% [...] = simulate_JakStat_RRE(tout,theta)
% [...] = simulate_JakStat_RRE(tout,theta,kappa,options)
% sol = simulate_JakStat_RRE(...)
% [status,tout,x,mx,y,sx,smx,sy] = simulate_JakStat_RRE(...)
%
% INPUTS:
% =======
...
function varargout = simulate_JakStat_RRE(varargin)
...
if(options_cvode.sensi>0)
    ...
    sol.yS = zeros(length(tout),11,length(options_cvode.sens_ind));
...
if(max(options_cvode.sens_ind)>18)
    error('Sensitivity index exceeds parameter dimension!')
end
cw_JakStat_RRE(sol,tout,theta(1:18),kappa(1:108),options_cvode,plist,pbar,xscale);
rt = [7  8  9  6  5  4  3  2  1];
sol.x = sol.x(:,rt);
...
if(nargout>1)
    ...
    varargout{2} = sol.t;
    varargout{3} = sol.x;
    varargout{4} = sol.y(:,1:9); % Moments of species
    varargout{5} = sol.y(:,10:end);
    if(nargout>5)
        varargout{6} = sol.sx;
        varargout{7} = sol.sy(:,1:9,:);
        varargout{8} = sol.sy(:,10:end,:);
    end
else
    sol.mx = sol.y(:,1:9);
    sol.y =  sol.y(:,10:end);
    sol.smx = sol.sy(:,1:9,:);
    sol.sy =  sol.sy(:,10:end,:);
    sol.theta = theta;
    sol.kappa = kappa;
    varargout{1} = sol;
end
